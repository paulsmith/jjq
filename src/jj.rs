...
/// Get parent change IDs for a revision.
#[allow(dead_code)]
pub fn get_parents(revset: &str) -> Result<Vec<String>> {
    let output = run_ok(&[
        "log",
        "-r",
...
/// Get the parent not reachable from trunk (for retry command).
pub fn get_candidate_parent(failed_rev: &str, trunk_bookmark: &str) -> Result<String> {
    let output = run_ok(&[
        "log",
        "-r",
        &format!("parents({}) ~ ::{}", failed_rev, trunk_bookmark),
        "--no-graph",
        "-T",
        "change_id.short()",
    ])?;
    let change_id = output.trim();
    if change_id.is_empty() {
        bail!(
            "could not find original candidate for failed revision {}",
            failed_rev
        );
    }
    Ok(change_id.to_string())
